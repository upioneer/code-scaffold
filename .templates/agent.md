...
Whenever modifying any code within the project (including `.skills`, `.templates`, or `scaffold.ps1`), you MUST execute these steps in order:
1.  Code Modification: Update the logic or content of the codebase.
2.  Metadata Update: For skills, parse the meta.json in the modified folder and increment the "version" property.
3.  Scaffold Update: If a change is detected within the .templates directory, you MUST review and update scaffold.ps1 to ensure the provisioning engine correctly handles the modifications.    
4.  Manifest Sync: Update the root manifest.json "version" and "lastUpdated" timestamp to match.
5.  Documentation: Update the project README.md (alphabetized) and create a walkthrough in project_details/history/v[NEW_VERSION]/readme.md.
6.  Update .skills\README.md to reflect all current skills in .skills directory.
7.  Commit & Push: Execute git add/commit/push only AFTER steps 1-6 are verified.
